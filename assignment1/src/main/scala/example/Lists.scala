...
object Lists:
  ...
  def sum(xs: List[Int]): Int =
    @tailrec
    def recurHelper(acc: Int, xs: List[Int]): Int =
      if xs.isEmpty then acc
      else recurHelper(acc + xs.head, xs.tail)
    recurHelper(0, xs)
  ...
  def max(xs: List[Int]): Int =
    ...
    def recurHelper(acc: Int, xs: List[Int]): Int =
      if xs.isEmpty then acc
        ...
        val newMax =
          if acc > xs.head then acc
          else xs.head
        return recurHelper(newMax, xs.tail)
    recurHelper(xs.head, xs.tail)
